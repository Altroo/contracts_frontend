...
jest.mock('@/components/pages/contracts/contract-form', () => ({
  ...
  default: (props: { session?: Session }) => {
    ...
    return React.createElement('div', null, `CONTRACT_FORM_CLIENT_MARKER:${JSON.stringify(props?.session ?? null)}`);
  },
}));
...
describe('ContractAddPage server component', () => {
  ...
  it('renders ContractFormClient with session when session exists', async () => {
    const sessionValue: Session = {user: {pk: 5, email: 'creator@site.com'}};
    ...
    expect(decoded).toContain('"pk":5');
    expect(decoded).toContain('"email":"creator@site.com"');
