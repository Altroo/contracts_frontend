...
jest.mock('@/components/pages/contracts/contracts-list', () => ({
	__esModule: true,
	default: (props: { session?: Session }) => {
		// eslint-disable-next-line @typescript-eslint/no-require-imports
		const React = require('react');
		return React.createElement('div', null, `CONTRACTS_LIST_CLIENT_MARKER:${JSON.stringify(props?.session ?? null)}`);
	},
}));
...
describe('ContractsListPage server component', () => {
	...
	it('renders ContractsListClient with session when session exists', async () => {
		const sessionValue: Session = { user: { pk: 10, email: 'admin@site.com' } };
