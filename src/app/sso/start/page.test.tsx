...
beforeEach(() => {
	...
	process.env.NEXT_PUBLIC_EBH_SSO_APP_SLUG = 'contracts';
	process.env.NEXT_PUBLIC_DOMAIN_URL_PREFIX = 'https://contracts.example.test';
});
...
describe('SSOStartPage', () => {
	it('redirects to central authorize with app metadata', () => {
		...
		expect(url.searchParams.get('redirect_uri')).toBe('https://contracts.example.test/sso/callback');
