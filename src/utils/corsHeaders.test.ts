 ...
 */
import {addCorsHeaders, getCorsHeaders} from './corsHeaders';
...
describe('getCorsHeaders', () => {
  ...
  it('sets Allow-Origin for a matching allowed origin (localhost in test env)', () => {
    // jest runs in test NODE_ENV so localhost:3001 is in allowedOrigins by default
    const headers = getCorsHeaders('http://localhost:3001') as Record<string, string>;
...
describe('addCorsHeaders', () => {
  it('returns a new Response with CORS headers added', () => {
    ...
    const result = addCorsHeaders(original, 'http://localhost:3001');
