...
import type { NotificationType, NotificationPreferenceType } from '@/types/contractNotificationTypes';
...
export const notificationApi = createApi({
	...
	endpoints: (builder) => ({
		getNotifications: builder.query<PaginationResponseType<NotificationType>, { page?: number }>({
			query: ({ page } = {}) => ({
				url: process.env.NEXT_PUBLIC_CONTRACTS_NOTIFICATIONS,
				method: 'GET',
				params: { page: page ?? 1 },
			}),
		...
		getNotificationPreferences: builder.query<NotificationPreferenceType, void>({
			query: () => ({
				url: process.env.NEXT_PUBLIC_CONTRACTS_NOTIFICATION_PREFERENCES,
				method: 'GET',
			}),
		...
		markNotificationsRead: builder.mutation<{ updated: number }, { ids?: number[] }>({
			query: (data) => ({
				url: process.env.NEXT_PUBLIC_CONTRACTS_NOTIFICATION_MARK_READ,
				method: 'POST',
				data,
			}),
		...
		getUnreadNotificationCount: builder.query<{ count: number }, void>({
			query: () => ({
				url: process.env.NEXT_PUBLIC_CONTRACTS_NOTIFICATION_UNREAD_COUNT,
				method: 'GET',
			}),
			providesTags: ['Notification'],
		}),
